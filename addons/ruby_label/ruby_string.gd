...
class UnbreakableWord:
	...
	func has_ruby() -> bool:
		return not ruby_units.is_empty()
	
	func get_phonetic() -> String:
		if ruby_units.is_empty():
			return word
		var r : String = ""
		for u in ruby_units:
...
class RubyString:
	...
	static func create(text : String,line_break : ILineBreakWord,
		...
		if parent.is_empty() or begin.is_empty() or end.is_empty():
...
class ILineBreakWord:
	func separate_word(text : String) -> PackedStringArray:
...
class LineBreakWord extends ILineBreakWord:
	...
	func is_link(c1 : String,c2 : String) -> bool:
		...
		var code1 = c1.unicode_at(0)
		var code2 = c2.unicode_at(0)
		if code1 <= 0x7F:
